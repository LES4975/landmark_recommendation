...
def getRecommendation(cosine_sim):
    ...
    return rec_movie_list[1:11] # 자기 자신(0번째)을 제외하고 1~10번째 영화를 출력

df_reviews = pd.read_csv('./dataset/LES/cleaned_reviews/only_reviews.csv')
tfidf_matrix = mmread('./models/tfidf_landmark_reviews.mtx').tocsr()
with open('./models/reviews_tfidf.pickle', 'rb') as f:
    tfidf = pickle.load(f)

# # 특정 콘텐츠의 인덱스를 이용한 추천
# ref_idx = 150 # 특정 영화의 인덱스
# print(df_reviews.iloc[ref_idx, 0])
# cosine_sim = linear_kernel(tfidf_matrix[ref_idx], tfidf_matrix) # 특정 영화의 벡터와 모든 영화의 벡터 간의 cosine 값을 구함
# print(cosine_sim) # 코사인 유사도
# print(len(cosine_sim[0])) # 영화 갯수만큼 나온다
